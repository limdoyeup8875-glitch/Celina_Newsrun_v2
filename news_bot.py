...
def summarize_with_llm(title, body):
    ...
    prompt = (
        "다음 뉴스 기사를 한국어 '개조식'으로 요약해줘. 리서치 노트에 쓸 거라 정보가 살아있어야 함.\n"
        "규칙:\n"
        "1) 불릿 정확히 2개. 각 불릿은 새 줄에서 '- ' 로 시작.\n"
        "2) 각 불릿은 '누가/무엇을/얼마나(숫자)'가 담긴 한 문장 분량의 정보를 넣되, "
        "문장 끝맺음만 명사형(개조식)으로 할 것. 즉 내용은 충실하게, 종결만 명사형.\n"
        "3) 끝맺음은 '~했다/~한다/~이다/~된다/~있다/~였다' 같은 서술형 금지. "
        "명사·명사형으로 맺어라(…체결/…확대/…추진/…전망/…돌입/…마련 등).\n"
        "4) 【중요】 제목을 그대로 반복하거나 'MOU 체결'처럼 앙상하게 압축하지 말 것. "
        "구체 주체·대상·규모·목적을 최소 하나는 포함.\n"
        "좋은 예: '- 삼성물산과 AI 스마트 빌딩 구축 위한 업무협약(MOU) 체결'\n"
        "나쁜 예: '- 업무협약 체결' (내용 없음)\n"
        "좋은 예: '- 의왕연구소 배터리동 화재로 그룹 차원 사고 대응책 마련'\n"
        "나쁜 예: '- 사고 대응 계획' (내용 없음)\n"
        "5) 서론·맺음말 없이 사실만, 숫자는 살려서.\n\n"
        f"[제목]\n{title}\n\n[본문]\n{body[:2500]}"
    )
    payload = {
        "model": GROQ_MODEL,
